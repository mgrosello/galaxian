...
table[32] = ([0 for i in range(INTERNAL_SIZE)])
...
for i in range(128):
	if table[i] is None:
		print( "\t{" + ", ".join("0" for j in range(INTERNAL_SIZE)) + "}%s\t/* not implemented or blank */" % ("," if i < 127 else ""))
	else:
		print( "\t{ /* '%c' */" % chr(i))
		for j in range(INTERNAL_SIZE):
			...
			h = hex(table[i][j])[2:]
			h = "0x" + ("0" * int (INTERNAL_SIZE / 4 - len(h))) + h
			print( "\t\t" + h + ",\t/* " + b + " */")
		print( "\t}" + ("," if i < 127 else ""))
