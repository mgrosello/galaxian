...
; Date: 20 August 2020. Keep checking for updates. 
; 
; Please send any questions, corrections and updates to scott.tunstall@ntlworld.com
;
; Be sure to check out my reverse engineering work for Robotron 2084 and Galaxian too, 
; at http://seanriddle.com/robomame.asm and http://seanriddle.com/galaxian.asm respectively.
;
; Finally:
...
CAN_DRAW_LANDSCAPE                  EQU $4110         ; Used in conjunction with CAN_DRAW_LANDSCAPE_2. Flag required for DRAW_LANDSCAPE to do its thing.
...
IS_MISSION_COMPLETE                  QU $4112         ; Flag. Set to $FF to complete the mission and run the MISSION_COMPLETED_SCRIPT (see $127E).  See also $22C9 and $232F
...
;
; See READ_LANDSCAPE_LAYOUT @$15C5 and DRAW_LANDSCAPE @ $0793 for detail.
;
; 
...
;
; Think of CIRC_SOUND_CMD_QUEUE_PTR_LO as the LSB of a pointer to where the next sound command requested will be queued, and 
...
;
; CHAR_BASED_GROUND_OBJECT are lightweight "projections" of the master GROUND_OBJECT record used for rendering character-based objects (targets!!) 
; sitting atop the landscape, i.e.: unlaunched rockets, fuel tanks, mystery, base. 
;
...
;
; GROUND_OBJECT holds the state for a single ground-based destructible target. i.e.: unlaunched rockets, fuel tanks, mystery, base
...
; you'll put it into an infinite loop.
0186: CD B2 01    call $01B2                 ; call CHECK_FOR_1PT_START1_HELD_DOWN
0189: 30 22       jr   nc,$01AD              ; if not held down, goto INFINITE_LOOP
018B: CD B2 01    call $01B2                 ; call CHECK_FOR_1PT_START1_HELD_DOWN
018E: 30 1D       jr   nc,$01AD              ; if not held down, goto INFINITE_LOOP

0190: 3E 01       ld   a,$01
0192: 32 01 68    ld   ($6801),a             ; enable interrupts

; Reset all scores in hi score table to 100.
...
; reset high score to 100 as well.
01A5: 21 AA 40    ld   hl,$40AA              ; load HL with address of last byte of HI_SCORE 
01A8: 36 01       ld   (hl),$01              ; set high score to be 100.
...
01DF: 30 02       jr   nc,$01E3              ; if carry is not set, then we have not reached the end of the queue ($4100), goto $202C
...
0299: DD 21 00 42 ld   ix,$4200              ; load IX with address of HI_SCORE_TABLE

; POKE last 2 digits of this high score to character RAM
029D: DD 7E 00    ld   a,(ix+$00)            ; get last 2 BCD digits of score (tens) into A
...
02D2: 28 01       jr   z,$02D5               ; write digit to character RAM
02D4: 77          ld   (hl),a                ; bump HL to character immediately below one just plotted

; Adjust HL to point to character two columns down. Remember the monitor is rotated 90 degrees.
02D5: 11 62 FF    ld   de,$FF62              ; load DE with -158
02D8: 19          add  hl,de                 ; bump HL to point to next 
...
; Display high score
03FC: 21 AA 40    ld   hl,$40AA              ; load HL with address of last byte of HIGH_SCORE              
...
0422: 28 08       jr   z,$042C               ; if the lower nibble is zero, goto $2281
...
0724: 38 02       jr   c,$0728               ; if A < 99 , return
0726: 3E 63       ld   a,$63                 ; clamp number of credits to 99 
...
0783: 28 0B       jr   z,$0790                 ; if high nibble is zero we don't care, goto $2580
...
0AD1: 1E 02       ld   e,$02                 ; Command ID: 5 = DISPLAY_SCORE_COMMAND, Param: 2 = Display player two's score
...
0F16: 1C          inc  e                     ; Command ID: 6 = PRINT_TEXT, Param:$16 = XI000 PTS
...
1915: 21 CC 19    ld   hl,$19CC              ; load HL with address of FUEL_TANK_ANIMATION_TABLE
1918: 18 08       jr   $1922

GROUND_OBJECT_MYSTERY_INIT:
191A: 21 D2 19    ld   hl,$19D2              ; load HL with address of MYSTERY_ANIMATION_TABLE
191D: 18 03       jr   $1922

GROUND_OBJECT_ROCKET_INIT:
191F: 21 C6 19    ld   hl,$19C6              ; load HL with address of ROCKET_ANIMATION_TABLE
...
19C6: 
    ...
    FF          ; end of animation marker 
    C6 19       ; pointer to $19C6, ROCKET_ANIMATION_TABLE 


FUEL_TANK_ANIMATION_TABLE:
19CC: 
    02 10 10    ; colour = 02, code = $10, delay = $10    
    FF          ; end of animation marker  
    CC 19       ; pointer to $19CC, FUEL_TANK_ANIMATION_TABLE  

MYSTERY_ANIMATION_TABLE:
19D2: 
    02 33 10    ; colour = 02, code = $33, delay = $10 
    FF          ; end of animation marker  
    D2 19       ; pointer to $19D2, MYSTERY_ANIMATION_TABLE 
    
BASE_ANIMATION_TABLE:    
19D8:      
    00 2F 06    ; colour = 0, code = $2F, delay = $06
    00 26 06    ; colour = 0, code = $26, delay = $06   
    00 1F 06    ; colour = 0, code = $1F, delay = $06
    FF          ; end of animation marker 
    D8 19       ; pointer to $19D8, BASE_ANIMATION_TABLE  
...
; Scroll the explosion off screen
1E14: DD 34 04    inc  (ix+$04)              ; increment INFLIGHT_ENEMY.X
1E17: C9          ret
...
2133: D6 06       sub  $06    
2135: 57          ld   d,a                   ; D = PLAYERS[0].X-9
...
; We've shot a fuel tank, add fuel to jet and award 50 pts
...
2289: 30 02       jr   nc,$228D              ; if there's no overflow after this add, goto $228D  
228B: 3E FF       ld   a,$FF                 ; otherwise, clamp to maximum amount of fuel you can have
228D: 77          ld   (hl),a                ; update CURRENT_PLAYER_FUEL 
228E: 11 03 03    ld   de,$0303              ; Command ID: 3 = UPDATE_PLAYER_SCORE_COMMAND, Param:1 = 50 pts
...
22E8: CD 46 23    call $2346                 ; call CHECK_IF_PLAYER_BULLET_HIT_ROCKET
22EB: FD 23       inc  iy
22ED: FD 23       inc  iy
